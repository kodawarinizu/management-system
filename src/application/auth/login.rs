...
impl LoginUseCase {
    ...
    pub async fn execute (&self, email: &str, password: &str) -> Result<Employee, DomainError> {
        let employee = self.repository.find_by_email(email)
            .await?
            .ok_or(DomainError::InvalidCredentials("Email or Password incorrect".to_string()))?;

        match employee.password_hash.verify(password)? {
            true => Ok(employee),
            false => Err(DomainError::InvalidCredentials("Email or Password incorrect".to_string())),
        }
    }
}